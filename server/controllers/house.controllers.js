    ...
    try {
        ...
        for (let i = 1; i <= 6; i++) {
            if (req.files?.[`display_image_${i}`]) {
                const result = await uploadImage(req.files[`display_image_${i}`].tempFilePath);
...
export const getQueryResults = async(req, res) => {
    try {
        ...
        if (req.body.features_climatizer === 'true') {
          query["features"] = {$elemMatch: {$eq: "central air conditioner / heating"}};
        }
        ...
        if (req.body.maxBathrooms) {
          if (!query.bathrooms) {
            ...
            query.bathrooms = { ...query.bathrooms, $lt: parseInt(req.body.maxBathrooms) };
          }
        }
        ...
        if (req.body.maxBedrooms) {
          if (!query.bedrooms) {
            ...
            query.bedrooms = { ...query.bedrooms, $lt: parseInt(req.body.maxBedrooms) };
          }
        }
        console.log(query)
